...
namespace QLyThuVien.Application.Features.Ai.Handlers;

public sealed class AiHandler :
    ApplicationRequestHandlerBase,
    IRequestHandler<AiSearchCommand, AiSearchResponse>,
    IRequestHandler<AiChatCommand, AiChatResponse>
{
    public AiHandler(ILibraryRepository repository, ICurrentUserContext currentUser, IClock clock)
        : base(repository, currentUser, clock)
    {
    }

    public async Task<AiSearchResponse> Handle(AiSearchCommand command, CancellationToken cancellationToken)
    {
    ...
    public async Task<AiChatResponse> Handle(AiChatCommand command, CancellationToken cancellationToken)
    {
        var request = command.Request;
