...
namespace QLyThuVien.Application.Features.Dashboard.Handlers;

public sealed class DashboardHandler : ApplicationRequestHandlerBase, IRequestHandler<GetDashboardSummaryQuery, DashboardSummaryDto>
{
    public DashboardHandler(ILibraryRepository repository, ICurrentUserContext currentUser, IClock clock)
        : base(repository, currentUser, clock)
    {
    }

    public Task<DashboardSummaryDto> Handle(GetDashboardSummaryQuery query, CancellationToken cancellationToken)
    {
